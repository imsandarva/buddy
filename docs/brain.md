...
Live talk is Gemini Live (`gemini-3.1-flash-live-preview`): raw mic PCM in, `Orion` (deep male) PCM out, no speech-to-text, no captions. SCREEN goes to Live as `realtimeInput` text whenever their screen changes (including while a job is walking the phone) and again when they start speaking. First meeting (onboarding activation) uses the same Live voice for one hello, then hangs up — never device TTS. Typed asks use the sheet and Android TTS. Mid-run questions on Live stay in the talk. See `docs/live.md` and `docs/live-jobs.md`.
